  ...
  getChildren(element?: ConfigItem): Thenable<ConfigItem[]> {
    if (element) {
      ...
      return Promise.resolve(
        groupProps.map((prop) => new ConfigItem(prop.key, prop.type, prop.default, prop.description))
      );
    }
    // 返回所有配置分组
    return Promise.resolve(
      Array.from(this.configGroups.keys()).map((group) => new ConfigItem(group, 'group', '', '', true))
    );
  }
...
class ConfigItem extends vscode.TreeItem {
  ...
  constructor(
    ...
    // 初始化TreeItem
    super(label, isGroup ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None);
...
export function registerConfigView(context: vscode.ExtensionContext): string {
  ...
  // 注册树视图
  context.subscriptions.push(vscode.window.registerTreeDataProvider('vssm-tool-config', configProvider));
  // 返回视图ID
